...
/* Determine the output redirection path, created by LH. */
  ...
  for(int i_lh = 0;i_lh < argc;i_lh++){
    if(argv[i_lh]){
      if((strcmp(argv[i_lh],"-o")==0)){
        i_lh++;
        int cc_end = strlen(argv[i_lh]);
        slime_bb_str = (char *)malloc(sizeof(char) * (cc_end + 3));
        memcpy(slime_bb_str, argv[i_lh], cc_end);
        strcat(slime_bb_str,".bb");
        break;
      }
    }else{
      break;
